...
public class DTAlgorithm extends Thread {

	private ArrayList<String> seniorities;
	private ArrayList<Developer> devs;
	private ArrayList<Task> tasks;
	private Map<Developer, ArrayList<Task>> result;

	public DTAlgorithm(ArrayList<String> seniorities,
			ArrayList<Developer> devs, ArrayList<Task> tasks) {
		this.seniorities = seniorities;
		this.devs = devs;
		this.tasks = tasks;
		this.result = new LinkedHashMap<Developer, ArrayList<Task>>();
		for (Developer d : this.devs) {
			this.result.put(d, new ArrayList<Task>());
		}
	}

	@Override
	public void run() {
		// this.result.put(this.devs.get(0), tasks);
		...
		do {
			...
			if (array.length > array[0].length) {
				System.out
						.println("Array transposed (because rows>columns).\n");
				array = transpose(array);
			}

			// <COMMENT> TO AVOID PRINTING THE MATRIX FOR WHICH THE ASSIGNMENT
			// IS CALCULATED
			...
			for (int i = 0; i < this.devs.size(); i++) {
				...
				ArrayList<Task> at = this.result.get(this.devs.get(i));
				at.add(this.tasks.get(assignment[i][1]));
				toRemove.add(this.tasks.get(assignment[i][1]));
	...
	private double[][] createArray() {
		...
		int lineLength = this.devs.size();
		int colLength = this.tasks.size();
		int difLine = 0;
		int difCol = 0;
		Seniority minor = Seniority.SENIOR_ANALYST;

		if (lineLength >= colLength) {
			difCol = (lineLength - colLength);
			array = new double[lineLength][colLength + (lineLength - colLength)];
		} else {
			difLine = (colLength - lineLength);
			array = new double[lineLength + (colLength - lineLength)][colLength];
		}
		
		for (int i = 0; i < this.devs.size(); i++) {
			Developer d = this.devs.get(i);
		...
		for (int i = 0; i < lineLength; i++) {
			...
			case INTERN:
				v = Double.parseDouble(this.seniorities.get(0));	
				vf = calculateVelocityByMinor(minor, v);
				fillLine(array, i, vf, difCol);		
				break;
			case TREINEE:
				v = Double.parseDouble(this.seniorities.get(1));
				vf = calculateVelocityByMinor(minor, v);
				fillLine(array, i, vf, difCol);
				break;
			case JUNIOR_ANALYST:
				v = Double.parseDouble(this.seniorities.get(2));
				vf = calculateVelocityByMinor(minor, v);
				fillLine(array, i, vf, difCol);
				break;
			case ANALYST:
				v = Double.parseDouble(this.seniorities.get(3));
				vf = calculateVelocityByMinor(minor, v);
				fillLine(array, i, vf, difCol);
				break;
			case SENIOR_ANALYST:
				v = Double.parseDouble(this.seniorities.get(4));
				vf = calculateVelocityByMinor(minor, v);
				fillLine(array, i, vf, difCol);
				break;
		...
		for (int j = 0; j < difLine; j++) {
			fillLine(array, lineLength + j, 200.0, difCol);
		}

		return array;
	}

	private double calculateVelocityByMinor(Seniority minor, double v) {
		double vminor = Double.parseDouble(this.seniorities.get(minor.getSeniority()-1));
		System.out.println("vminor = " + vminor);
		double vf = v/vminor;
		System.out.println("Velo final = " + vf);
		return vf;
	}

	private void fillLine(double[][] array, int i, double v, int difCol) {
		double maior = 0;
		for (int j = 0; j < this.tasks.size(); j++) {
			Task t = this.tasks.get(j);
			double temp = t.getTimeTask() * v;
			array[i][j] = temp;
			if (temp > maior) {
				maior = temp;
			}
		...
		for (int j = 0; j < difCol; j++) {
			array[i][this.tasks.size() + j] = maior;
		}
	}

	public Map<Developer, ArrayList<Task>> getResult() {
		return result;
	}
